...
import erogenousbeef.bigreactors.common.item.Mallorium;
import erogenousbeef.bigreactors.common.item.Chipset;
...
public class CommonProxy {
	...
	private void sendInterModAPIMessages() {
		...
		final int CYANITE = 1;
		final int GRAPHITE = 2;
		final int BLUTONIUM = 3;
		final int LUDICRITE = 4;
		final int MALLORIUM = 5;
		final int MIXEDREACTANT = 6;
		...
		if(Loader.isModLoaded("ThermalExpansion")) {
			...
			ItemStack ingotMixedReactant = ingots[MIXEDREACTANT].copy();
			...
			if(ingots[YELLORIUM] != null) {
				...
				//ThermalExpansionHelper.addSmelterRecipe(1600, yelloriteOre, malloriteOre, ingots[MALLORIUM]);
			}
			if(ingots[MIXEDREACTANT] != null) {
				ThermalExpansionHelper.addSmelterRecipe(1600, yelloriteOre, malloriteOre, ingots[MIXEDREACTANT]);
			}
