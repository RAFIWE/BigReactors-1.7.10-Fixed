...
import erogenousbeef.bigreactors.common.block.BlockBROreM;
...
public class BigReactors {
    ...
    public static Block blockMalloriteOre;
    ...
    public static Mallorium fluxMallorium;
    public static Chipset reactorChip;
    public static ItemBase Panels;

    public static BRSimpleOreGenerator yelloriteOreGeneration;
    public static BRSimpleOreGenerator malloriteOreGeneration;
    ...
    private static boolean registerMelloriteSmeltToIngot = true;
    ...
    public static void register(Object modInstance) {
        ...
        if (!INITIALIZED) {
            ...
            registerMelloriteSmeltToIngot = BRConfig.CONFIGURATION.get("Recipes", "registerMelloriteSmeltToIngot", true, "If set, yellorite ore will smelt into whichever item is registered as ingotUranium in the ore dictionary. If false, it will smelt into ingotMellorite. (Default: true)").getBoolean(true);
            ...
            String malloriumIngot = "ingotMallorium";
            String mixedReactantDust = "dustMixedReactant";
            String mixedReactantIngot = "ingotMixedReactant";
            ...
            // Mallorium
            if (blockMalloriteOre != null) {
                ItemStack product;

                if (registerMelloriteSmeltToIngot) {
                    ArrayList<ItemStack> candidateOres = OreDictionaryArbiter.getOres("ingotMallorium");
                    if (candidateOres == null || candidateOres.size() <= 0) {
                        BRLog.warning("Config value registerMelloriteSmeltToIngot is set to True, but there are no ores registered as ingotUranium in the ore dictionary! Falling back to using standard mallorium only.");
                        candidateOres = OreDictionaryArbiter.getOres("ingotMallorium");
                    }
                    product = candidateOres.get(0).copy();
                } else {
                    product = OreDictionaryArbiter.getOres("ingotMallorium").get(0).copy();
                }

                GameRegistry.addSmelting(blockMalloriteOre, product, 0.5f);
            }
            ...
            GameRegistry.addRecipe(new ShapedOreRecipe(blockMetal.getItemStackForMaterial("Mallorium"), "BBB", "BBB", "BBB", 'B', malloriumIngot));
            GameRegistry.addRecipe(new ShapedOreRecipe(blockMetal.getItemStackForMaterial("MixedReactant"), "BBB", "BBB", "BBB", 'B', mixedReactantIngot));
            GameRegistry.addRecipe(new ShapedOreRecipe(fluxMallorium, "ICI", "CUC", "ICI", 'U', Items.diamond, 'C', malloriumIngot, 'I', mixedReactantDust));
            GameRegistry.addRecipe(new ShapedOreRecipe(reactorChip, "CUC", "UBU", "CUC", 'U', IC2Items.getItem("iridiumOre"), 'C', mixedReactantIngot, 'B', Items.blaze_powder));


            //String Mallorium = new ShapedOreRecipe(blockMetal.getItemStackForMaterial("Mallorium");
            ...
            if (blockReactorPart != null) {
                ...
                GameRegistry.addRecipe(new ShapedOreRecipe(reactorPartStack, "C C", "GDG", "CRC", 'D', fluxMallorium, 'G', yelloriumIngot, 'C', "reactorCasing", 'R', Items.redstone));
...
//    }
    ...
    public static ItemStack registerOres(int i, boolean b) {
        ...
        if (blockMalloriteOre == null) {
            blockMalloriteOre = new BlockBROreM();
            GameRegistry.registerBlock(BigReactors.blockMalloriteOre, ItemBlockBigReactors.class, "MalloriteOre");
            ItemStack malloriteStack = new ItemStack(blockMalloriteOre, 1);
            OreDictionary.registerOre("oreMallorium", malloriteStack);
            OreDictionary.registerOre("oreMallorium", malloriteStack); // For convenience of mods which fiddle with recipes
        }
        ...
        boolean genMalloriteOre = BRConfig.CONFIGURATION.get("WorldGen", "GenerateMalloriteOre", true, "Add mallorite ore during world generation?").getBoolean(true);
        if (malloriteOreGeneration == null && genMalloriteOre) {
            // Magic number: 1 = stone
            int clustersPerChunk;
            int orePerCluster;
            int maxY;

            clustersPerChunk = BRConfig.CONFIGURATION.get("WorldGen", "MaxMalloriteClustersPerChunk", 2, "Maximum number of clusters per chunk; will generate at least half this number, rounded down").getInt();
            orePerCluster = BRConfig.CONFIGURATION.get("WorldGen", "MaxMalloriteOrePerCluster", 3, "Maximum number of blocks to generate in each cluster; will usually generate at least half this number").getInt();
            maxY = BRConfig.CONFIGURATION.get("WorldGen", "MalloriteMaxY", 30, "Maximum height (Y coordinate) in the world to generate mallorite ore").getInt();
            int[] dimensionBlacklist = BRConfig.CONFIGURATION.get("WorldGen", "MalloriteDimensionBlacklist", new int[]{}, "Dimensions in which Mallorite ore should not be generated; Nether/End automatically included").getIntList();

            malloriteOreGeneration = new BRSimpleOreGenerator(blockMalloriteOre, 0, Blocks.stone,
                    clustersPerChunk / 2, clustersPerChunk, 4, maxY, orePerCluster);

            // Per KingLemming's request, bonus yellorite around y12. :)
            BRSimpleOreGenerator malloriteOreGeneration2 = new BRSimpleOreGenerator(blockMalloriteOre, 0, Blocks.stone,
                    1, 2, 11, 13, orePerCluster);

            if (dimensionBlacklist != null) {
                for (int dimension : dimensionBlacklist) {
                    malloriteOreGeneration.blacklistDimension(dimension);
                    malloriteOreGeneration2.blacklistDimension(dimension);
                }
            }

            BRWorldGenerator.addGenerator(BigReactors.malloriteOreGeneration);
            BRWorldGenerator.addGenerator(malloriteOreGeneration2);
        }
    ...
    public static ItemStack registerIngots(int id) {
        if (BigReactors.ingotGeneric == null) {
            ...
            if (registerYelloriumAsUranium) {
                ...
                OreDictionary.registerOre("ingotMallorium", ingotGeneric.getItemStackForType("ingotMallorium"));
                OreDictionary.registerOre("ingotMixedReactant", ingotGeneric.getItemStackForType("ingotMixedReactant"));
                ...
                OreDictionary.registerOre("dustMallorium", ingotGeneric.getItemStackForType("dustMallorium"));
            }
    ...
    public static void registerItems() {
        ...
        if (fluxMallorium == null) {
            fluxMallorium = new Mallorium();
            GameRegistry.registerItem(fluxMallorium, "MalloriumCrystal");
            //   GameRegistry.addRecipe(new ShapedOreRecipe(blockMetal.getItemStackForMaterial("Ludicrite"), "BPB", "ENE", "BPB", 'N', Items.nether_star, 'P', Items.ender_pearl, 'E', Blocks.emerald_block, 'B', blutoniumIngot));
        }
        if (reactorChip == null) {
            reactorChip = new Chipset();
            GameRegistry.registerItem(reactorChip, "ReactorChipset");
        }
    }
    ...
    public static void registerNonBlockFluidIcons(TextureMap map) {
        ...
        //Malorium = map.registerIcon(TEXTURE_DIRECTORY + "blocks/tile.brMetal.blockMixedYellorium.png");
